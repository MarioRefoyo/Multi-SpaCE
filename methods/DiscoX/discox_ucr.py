...
from discover import discords_discox as discords
import pandas as pd
import sys
...
steps = np.concatenate([ np.arange(0.01, 0.5, 0.01),
...
for X_i, X in enumerate(X_test):
    ...
    for target_c in np.unique(y_train):
        ...
        X_target_c_sep = []
        for X_ in X_target_c:
            X_target_c_sep.append(list(X_))
            X_target_c_sep.append([np.nan])
        X_target_c_sep = [
            item for sublist in X_target_c_sep for item in sublist][:-1]
        X_target_c_sep = np.asarray(X_target_c_sep)

        print('X_i, target_c', X_i, target_c)

        mp_ = stumpy.stump(X, window, X_target_c_sep,
                           ignore_trivial=False, normalize=False)

        mp = mp_[:, 0].astype(float)
        mpi = mp_[:, 1].astype(int)

        mock_mp = {'data': {'ts': X, 'query': X_target_c_sep},
                   'class': 'MatrixProfile', 'metric': 'euclidian',
                   'mp': mp, 'mpi': mpi, 'w': window,
                   'motifs': [], 'discords': []}
        ...
        while n_non_mapped and max_len_non_mapped >= 3 and not cf_found:
            mock_mp = discords(mock_mp, k=1, recurs=True, exclusion_zone=0)
            
            # ...or until there are no more discords
            if not len(mock_mp.get('discords')):
                print('No more discords')
                break

            for d_pair in mock_mp.get('discords'):
                ...
                d_X_start, d_X_end = d_pair[0][1], d_pair[0][1] + window
                d_nun_start, d_nun_end = d_pair[1][1], d_pair[1][1] + window
                ...
                if len(X_inter):
                    # length of current subsequence
                    curr_len = d_X_end-d_X_start
                    
                    if len(X_inter) == curr_len:
                        continue
                    ...
                    if len(ind_list_to_intervals(X_inter)[0]) == 1:
                        ...
                        if X_inter[-1]+1 in np.arange(d_X_start, d_X_end):
                            ...
                            exp_len = int(min(to_curr_start, curr_len-len(X_inter)))
                                     
                            mapped[d_X_start:d_X_start + exp_len] = np.arange(
                                mapped[exp_from] - exp_len, mapped[exp_from])
                                                            
                            nun_subs = X_target_c_sep[int(mapped[exp_from]) - exp_len:int(mapped[exp_from])]
                            target_ts[d_X_start:d_X_start + exp_len] = nun_subs
                        ...
                        if when_left < when_right:
                            ...
                            if to_curr_end >= curr_len - len(X_inter) + adj_len:
                                exp_len = curr_len - len(X_inter) + adj_len
                            else:
                                exp_len = int(min(to_curr_end, curr_len-len(X_inter)))
                        ...
                        #expand from right to left
                            ...
                            # print('exp_from, mapped[exp_from]', exp_from,
                            #       mapped[exp_from])
                            ...
                            if to_curr_start >= curr_len + adj_len:
                                exp_len = curr_len - len(X_inter) + adj_len
                            else:
                                exp_len = int(min(to_curr_start, curr_len-len(X_inter)))
                            ...
                            nun_subs = X_target_c_sep[int(mapped[exp_from]) - exp_len:int(mapped[exp_from])]
                            target_ts[exp_from - exp_len:exp_from] = nun_subs                                
                    ...
                    nun_subs = X_target_c_sep[d_nun_start:d_nun_end]
                   
                    target_ts[d_X_start:d_X_end] = nun_subs
                    mapped[d_X_start:d_X_end] = np.arange(d_nun_start, d_nun_end)          
                    when_mapped[d_X_start:d_X_end] = np.full_like(nun_subs, fill_value=when)
                    when += 1
                
                print(mapped)
                
                try:
                    _, target_ts = fill_short_intervals(mapped, target_ts,
                                                 X_target_c_sep, thres=3)
                except Exception as ex:
                    print('Could not fill short', ex)
            
                #exclude current index from next searches
                mock_mp['mp'][d_X_start] = -np.inf
                    
                for w_i_, w in enumerate(steps): 
                    X_cf = (1-w)*X + w*target_ts
                    X_cf = interpolate_short(X_cf, mapped, thres=3)
