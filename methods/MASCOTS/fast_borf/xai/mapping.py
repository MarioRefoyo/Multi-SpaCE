...
from fast_borf.xai.sax_mapping import (
    align_sax_words_to_raw_ts,
    wsax_configurations_alignment_conversion,
)
...
class BagOfReceptiveFields:
    ...
    def build(
        ...
        self.receptive_fields_, _ = build_receptive_fields(
            X=X,
    ...
    def add_feature_importance(self, F):
        ...
        if self.task_ == "classification":
            if F.ndim == 2:  # this happens in binary classification
                ...
                F = np.concatenate(
                    [-F[np.newaxis, ...], F[np.newaxis, ...]], axis=0
                )
    ...
    def map_contained_feature_importance_to_saliency(
        ...
        for i in range(len(self.X_)):
            S_single = (
                self.map_contained_single_feature_importance_to_saliency(
    ...
    def get_most_important_contained_patterns_by_signal(self, i):
        ...
        for signal in range(num_signals):
            # Identify indices within the current signal
            signal_indices = not_zero_indices[
                signal_idxs[not_zero_indices] == signal
            ]
        ...
        for i in range(len(X)):
            if word_idx in sax_converted_X[conf_idx][i][signal_idx]:
                ...
                ts_receptive_fields_mappings.append(
                    np.array(X[i, signal_idx][align])
                )
            else:
                ts_receptive_fields_alignments.append(
                    np.empty(
                        (0, word_length, window_size // word_length),
                        dtype=np.int_,
                    )
                )
                ts_receptive_fields_mappings.append(
